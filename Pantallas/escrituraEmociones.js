...
import { agregarEmocion } from '../BaseDeDatos/fireBaseCRUD';
import AsyncStorage from '@react-native-async-storage/async-storage'; 



const EscrituraEm = (props) => {
    ...
    const guardarEmocion = async() => {
        ...
        if(state.nombre === '')
        {
            alert('Nombre su emoción por favor')
            return;
        }
        else{
            agregarEmocion(state.nombre, state.descripcion, state.color, usuarioId)
         .then(id => {
        console.log("Emocion agregada con ID:", id);
  })
  .catch(error => {
    ...
    return (
        <View style={estilos.container}> 
          ...
          <TextInput placeholder="Nombre de emoción"
            ...
            <View>
                <Button title="Guardar Emocion" onPress={guardarEmocion}/>
            </View>
          </View>
